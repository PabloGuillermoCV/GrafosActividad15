...
public class Heap<K,V> implements PriorityQueue<K, V> {
	...
	/* RECORDAR: sea i la posicion actual en la Heap...
	 * Padre(i) = i/2
	 * Hijo_izquierdo(i) = 2i
	 * Hijo_derecho(i) = 2i+1
	 */
	...
	public Heap (int maxElems, Comparator<K> comp){
		size = 0;
		ArbolHeap = (Entrada<K,V>[]) new Entrada[maxElems];
		this.comp = comp;
	}
	/**
	...
	public Entry<K,V> insert(K key,V value)throws InvalidKeyException{
		...
		while (i > 1 && seguir){
			...
			if(comp.compare(EA.getKey(), padre.getKey()) < 0){ //el hijo es mas chico que el Padre, esto es una MinHeap, asi que NO Puede pasar
	...
	public Entry<K,V> removeMin()throws EmptyPriorityQueueException{
		...
		if(size == 1){
			...
			boolean seguir = true;
			while(seguir){
				//calculo los hijos de la posicion en la que estoy
				int HI = 2*i;
				int HD = (2*i)+1;
				boolean EHI = HI <= size(); //determino si realmente existen los hijos de la posicion
				boolean EHD = HD <= size();
				if(!EHI) //si no hay Hijo Izquierdo, llegue a una hoja y ya movi todo de forma ordenada, no hace falta seguir
					seguir = false;
				else{
					...
					if(comp.compare(ArbolHeap[i].getKey(), ArbolHeap[m].getKey()) > 0){ //si el minimo de los hijos es mas grande que el padre
						...
						ArbolHeap[m] = aux;
						i = m; //sigo con "m" siendo la nueva posicion
					}
					else
						seguir = false; //el minimo de los hijos es mas chico, ya ordene la Heap
				}
			}
		}
		return ret;
